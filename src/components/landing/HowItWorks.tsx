    ...
    <section id="how" className="py-24 bg-cream-warm">
      <div className="container max-w-6xl">
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {cards.map((c) => (
            <div key={c.n} className="rounded-3xl border border-ink/10 bg-white p-7 flex flex-col">
              <div className="flex items-center justify-between mb-6">
                <div className={`h-11 w-11 rounded-2xl flex items-center justify-center ${c.iconBg}`}>
                  <c.icon className="h-5 w-5" />
                </div>
                <span className="font-display text-sm text-ink-muted">{c.n}</span>
              </div>
              <h3 className="font-display font-bold text-xl text-ink leading-tight">{c.title}</h3>
              <p className="mt-2 text-sm text-ink-muted flex-1">{c.body}</p>
