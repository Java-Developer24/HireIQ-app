        ...
        <div className="max-w-[680px] w-full bg-white border border-ink/10 rounded-[32px] p-10 shadow-sm space-y-10 animate-in slide-in-from-bottom-4 duration-500">
           <div className="space-y-2">
              <div className="flex items-center justify-between mb-4">
                <span className="text-[10px] font-bold text-ink-muted uppercase tracking-widest">Question {currentQuestion + 1} of {questions.length}</span>
                <div className="flex items-center gap-2">
                  <div className="w-32 h-1.5 bg-cream rounded-full overflow-hidden">
                    <div className="h-full bg-forest transition-all duration-500" style={{ width: `${((currentQuestion + 1) / questions.length) * 100}%` }} />
                  </div>
                  <div className="flex items-center gap-2 text-xs font-bold text-ink-muted shrink-0">
           ...
           <div className="space-y-4 pt-4">
             <button
               ...
               className={`w-full h-12 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition shadow-lg ${
                 selected ? "bg-[#0D1829] hover:bg-forest-deep shadow-[#0D1829]/10" : "bg-ink/10 cursor-not-allowed shadow-none"
               }`}
             >
